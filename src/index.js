    ...
    var MicroBar = React.createClass({
      ...
      renderBarChart: function() {
        ...
        bar.append("rect")
          ...
          .attr("xlink:href", this.props.xlink) // added
          .on("mouseover", function (d, i) {
            var box, coords, fadingIn, matrix, point, scrollLeft, scrollTop, tipHeight, tipWidth;
            d3.select(this).attr("fill", self.props.hoverColor(d, i)); //modded, added d,i args
            if (self.props.tooltip) {
          ...
          .on("mouseout", function (d, i) {
            d3.select(this).attr("fill", self.props.fillColor(d, i)); //modded, added d,i args
            if (self.props.tooltip) {
              return tooltip.transition().duration(250).style("opacity", 1 / 1e6).each("end", function () {
                return tooltip.style("visibility", "hidden");
              });
            }
          }).on("click", function (d, i) { //added 
            var url = self.props.xlink(d, i);
            if (url !== '') {
              window.open(url);
            }
          });
